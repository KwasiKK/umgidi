...
class ProductModel
{
    ...
    {
        $sql = "SELECT user_id, product_id, product_text FROM products WHERE user_id = :user_id";
	...
	$sql = "INSERT INTO product (product_name, brandname, category,flavour, unit_volume, image_url) 
	VALUES ('".strip_tags($_POST['product_name'])."', '"
		.strip_tags($_POST['brandname'])."', '"
		.strip_tags($_POST['category'])."', '"
		.strip_tags($_POST['flavour'])."', '"
		...
		$sql = "INSERT INTO stock (stock_owner, back_quantity, front_quantity, cost_price, selling_price, mark_up_amount) 
		VALUES ('".strip_tags($_POST['stock_owner'])."', '"
			.strip_tags($_POST['back_quantity'])."', '"
			.strip_tags($_POST['front_quantity'])."', '"
		...
		if ($count == 1) {
            ...
            $_SESSION["feedback_negative"][] = "Product insert error ".FEEDBACK_NOTE_CREATION_FAILED;
        }
        // default return
        return false;
    }
